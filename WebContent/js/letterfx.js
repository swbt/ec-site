(function($) {
	...
	LetterFx.prototype.init = function() {
		...
		this.new_html = this.$element.text().replace(this.options.pattern,
				this.options.replacement);

		this.$element.addClass(this.options.css.element.base).addClass(
				this.options.css.element.before);

		this.$element.html(this.new_html);

		this.$letters = this.$element.find(this.options.selector);

		this.$letters.css('transition-duration', this.options.fx_duration)
				.addClass(this.options.css.letters.base).addClass(
						this.options.css.letters.before);
	...
	LetterFx.prototype.bindLetterFxEnd = function() {
		...
		this.$letters.bind("transitionend", function() {
			...
			case "rewind":
				lfx.applyLetterFx($(this), options.timing,
						options.css.letters.after, options.css.letters.before);
				break;
	...
	LetterFx.prototype.terminate = function() {
		...
		default:
			...
			this.$element.removeClass(this.options.css.element.base)
	...
	LetterFx.prototype.startKillWatch = function() {
		var fx_duration = this.options.fx_duration.match(/\d+s/) ? parseInt(this.options.fx_duration)
				: 1;
		var time = Math.ceil(1.5 * this.num_letters * this.options.timing
				* fx_duration);
		var lfx = this;
	...
	LetterFx.prototype.fx = function() {
		var lfx = this;
		this.startKillWatch();
		this.$element.removeClass(this.options.css.element.before).addClass(
				this.options.css.element.after);
		var $letters = this.options.sort(this.$letters);
		var options = this.options;

		$letters.each(function(i, letter) {
			lfx.applyLetterFx($(letter), (i + 1) * options.timing,
					options.css.letters.before, options.css.letters.after);
		});
		return this;
	}

	LetterFx.prototype.applyLetterFx = function($letter, timing, css_before,
			css_after) {
		var options = this.options;
	...
	LetterFxConfig.prototype.buildCss = function(flip) {
		...
		css.element.base = options.element_class
				+ "-container "
				+ options.fx.replace(/(\S+)/g, options.element_class
						+ "-$1-container");
		css.element[before] = options.fx.replace(/(\S+)/g,
				options.element_class + "-$1-before-container");
		css.element[after] = options.fx.replace(/(\S+)/g, options.element_class
				+ "-$1-after-container");

		css.letters.base = options.element_class;
		css.letters[before] = options.fx.replace(/(\S+)/g,
				options.element_class + "-$1-before")
		css.letters[after] = options.fx.replace(/(\S+)/g, options.element_class
				+ "-$1-after")
	...
	$.fn.letterfx = function(config) {
		...
		return $(this).each(
				function() {
					var $element = $(this);
					if (!$element.data('letterfx-obj')
							|| $element.data('letterfx-obj').isDone()) {
						$element.data('letterfx-obj', new LetterFx($element,
								config));
					}
				});
	...
	$.fn.letterfx.defaults = {
		...
		// fx, like fly or fade, can happen in (eg fade-in) or out (eg
		// fade-out).
		// The default is in; change backwards to true to reverse the order of
		// the effects.
		backwards : false,
		...
		// letter fx start sequentially: letters start their fx one after
		// another.
		// this sets time between the letters
		timing : 50,

		// duration of each fx
		// options the same as css property for transition-duration ("1ms",
		// "1s", etc)
		fx_duration : "1s",
		...
		// what to do when the entire element has completed all its letter
		// effects
		// options include:
		...
		// options: true, false, "element" ("element" waits until all letters
		// complete fx before restoring)
		restore : true,

		// destroy element/letters after fx, useful on {out:true} fx
		// options: true, false, "letters" ("letters" destroys each letter after
		// fx, but elements original content may be restored after all letters
		// complete fx)
		destroy : false,
